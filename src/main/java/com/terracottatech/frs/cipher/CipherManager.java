 ...
 *
 * <p>CipherManager provides methods to encrypt and decrypt data stored in ByteBuffers, allowing
...
public interface CipherManager {
  Logger LOGGER = LoggerFactory.getLogger(CipherManager.class);

  /**
   * Validates if the provided encryption algorithm is supported by the JVM.
   *
   * <p>This method checks whether the specified algorithm is available for use
   * with the Cipher class in the current Java environment. It attempts to create
   * a Cipher instance with the given algorithm name.
   *
   ...
   */
  static boolean validateAlgorithm(String algorithm) {
    // Input validation
   ...
   *
   * <p>An initialization vector is a random or pseudo-random value that is used along with
   ...
   *
   * <p>The generated IV should be stored alongside the encrypted data as it will be required
   * for decryption.
   *
